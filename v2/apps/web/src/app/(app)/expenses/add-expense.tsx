...
export default function AddExpense({ onClose, onDone, page }: {
  onClose: () => void;
  onDone: () => void;
  /** Full screen with a back arrow, for /expenses/new — a form the phone's
      back gesture closes instead of closing the app. */
  page?: boolean;
}) {
  ...
  return (
    <div className={page
      ...
      <div className={page
        ? 'bg-white w-full h-full flex flex-col overflow-y-auto'
        : 'bg-white w-full sm:max-w-[460px] sm:rounded-xl rounded-t-2xl shadow-xl max-h-[92vh] overflow-y-auto'}
        onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2 px-4 sm:px-5 h-[56px] border-b border-line-soft sticky top-0 bg-white z-10">
          {page && (
            <button onClick={onClose} aria-label="Back" className="-ml-2 p-2 text-ink-2">
              <Icon name="chevRight" size={20} className="rotate-180" />
            </button>
          )}
          <h2 className="text-[16px] font-semibold flex-1 truncate">Add an expense</h2>
          {!page && (
            <button onClick={onClose} aria-label="Close"
              className="w-8 h-8 rounded hover:bg-wash flex items-center justify-center">
              <Icon name="x" size={16} />
            </button>
          )}
        </div>
        ...
        <div className="p-5 flex flex-col gap-3.5 pb-[max(1.25rem,env(safe-area-inset-bottom))]">
          ...
          {report && (report.found
            ...
            : <div className="rounded-lg border border-line bg-wash px-3 py-2 text-[12.5px] text-muted">
                No folder open for this day yet — this expense opens one.
              </div>)}
          ...
          <button onClick={submit} disabled={!canAdd || busy}
            className="h-12 rounded-xl lg:rounded-md bg-accent text-white text-[15px] lg:text-[14px]
              font-bold hover:brightness-90 active:brightness-90 disabled:opacity-50">
