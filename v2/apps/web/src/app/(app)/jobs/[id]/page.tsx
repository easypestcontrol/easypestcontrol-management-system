...
import { canRecordService, isFieldTech, money, toMin } from 'shared';
...
import TechRun from './run';
import TechDone from './done';
...
function TechDetail({ j, me, reload }: {
  ...
  // Only the head records the work; the crew get the trip and a read-only
  // view. Asked of `shared` so this matches the server exactly — a screen that
  // offers what the API refuses is worse than one that offers nothing.
  ...
  if (done) {
    return (
      <>
      {/* The phone reads the finished service as a report; the desk keeps the
          panels it has always had. */}
      <TechDone j={j} />
      <div className="max-lg:hidden p-5 max-w-[640px] mx-auto pb-16">
        ...
        {finished && (
          <Modal title="Service completed" sub={j.id + ' · ' + j.title}
            ...
            <button onClick={() => setFinished(null)}
              className="mt-5 w-full h-12 lg:h-10 rounded-xl lg:rounded bg-accent text-white
                text-[15px] lg:text-[13px] font-semibold hover:brightness-90">
              Done — report sent
            </button>
          </Modal>
        )}
  ...
  /* The phone runs the service a step at a time; the desk keeps the stack.
     Same eleven steps, same endpoints — a different shape for a different
     way of holding the thing. */
  return (
    <>
    <TechRun j={j} me={me} reload={reload} />
    <div className="max-lg:hidden p-5 max-w-[640px] mx-auto pb-24">
      <BackLink label="Today's work" />
      <div className="mt-3">
        <HeaderCard j={j} />
      </div>
    ...
    <div className="card p-4">
      ...
      {(cl?.phone || cl?.addr || cl?.city) && (
        <div className="grid grid-cols-2 gap-2 mt-3.5">
          ...
          {/* Nothing to navigate to once the work is done — offering it there
              only started a fresh trip out of a finished service. */}
          {(cl?.addr || cl?.city) && j.status !== 'completed' && <NavigateButton j={j} />}
        </div>
      )}
      {j.notes && (
