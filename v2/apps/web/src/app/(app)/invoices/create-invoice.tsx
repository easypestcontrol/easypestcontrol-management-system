...
/* ============================================================================
   Raise an invoice — against a contract, or standalone.

   It lives in its own module rather than inside the invoices page because a
   Next route file may only export its default; the phone opens this at
   /invoices/new as a screen, and the desktop still opens it as a dialog over
   the list.
   ========================================================================== */
...
import {
  Dialog, STATUS_LABEL, fmtDate, todayISO,
  type ContractOption, type InvoiceDetail,
} from './ui';
...
export function CreateDialog({ onClose, onCreated, page }: {
  onClose: () => void; onCreated: (id: string) => void; page?: boolean;
}) {
  ...
  const step2 = !!page && mode === 'contract' && !!sel;

  return (
    <Dialog
      /* On a phone this is two screens, not one long scroll: choose the
         contract, then price it. The back arrow steps back to the list of
         contracts rather than throwing away the whole invoice. */
      title={step2 ? (options?.find((o) => o.id === sel)?.clientName || 'Invoice') : 'New invoice'}
      sub={step2 ? sel : undefined}
      wide page={page} onClose={step2 ? () => setSel('') : onClose}
      footer={
        <>
      ...
      {/* Mode switch. Hidden once a contract is chosen on the phone — by
          then the question "contract or standalone" has been answered. */}
      <div className={'flex gap-1 rounded-md bg-wash p-1 mb-4 w-fit ' + (step2 ? 'hidden' : '')}>
